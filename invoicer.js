...
function markOrderAsInvoiced(orderId) {
    try {
        ...
        setDoc(doc(db, "orders", orderId), {
            ...
            alert(orderId + " marked as invoiced.")
            window.location.reload();
        })
    } catch (error) {
        alert(`Order not marked`)
        console.error(error);
    }
        ...
        .then((docs) => {
            ...
            docs.forEach(order => {
              let rushStatus = order.data().rush ? " rush" : ""
                let orderLink = `<tr class="${order.data().status}${rushStatus}"><td><a href="./order.html?id=${order.id}">${order.id}</a></td><td>${order.data().totalQuantity}</td><td>${order.data().productType}</td><td>${order.data().orderNotes?order.data().orderNotes:""}</td><td>${order.data().upc?"UPC":"-"}</td><td>${order.data().approvalDate ? order.data().approvalDate : "-"}</td><td>${order.data().dueDate ? order.data().dueDate : "-"}</td><td><button class="invoice-button" data-invoice-order-id="${order.id}" >Invoice</button></td></tr>`
